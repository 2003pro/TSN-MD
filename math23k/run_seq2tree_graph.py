...
sftarget_path = 'data/node_seq_757.pt'
node = torch.load(sftarget_path)
node_idx_path = 'data/idx_seq_757.json'
...
for epoch in range(n_epochs):
   ...
   num_stack_batches, num_pos_batches, num_size_batches, num_value_batches, graph_batches, idx_batches = prepare_train_batch(train_pairs, batch_size)
    ...
    for idx in range(len(input_lengths)):
        loss,node_out = train_tree(
            input_batches[idx], input_lengths[idx], output_batches[idx], output_lengths[idx],
            num_stack_batches[idx], num_size_batches[idx], generate_num_ids, encoder, predict, generate, merge,
            encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer, output_lang, num_pos_batches[idx], graph_batches[idx],idx_batches[idx],sf_dic)
        loss_total += loss
    ...
    if epoch % 2 == 0 or epoch > n_epochs - 5:
        ...
        if value_ac > best_acc:
            print('[saving .....]')
            print('len(idx_batches)')
            print(len(idx_batches))
            print('len(all_node_out)')
            print(len(all_node_out))
            best_acc = value_ac
            best_seq_path = best_path + str(value_ac) + '.json'
